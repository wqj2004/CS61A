...
def dfs(curs, s0, s1, d):
    ret = 0
    if len(curs) == 1:
        if abs(s0 + curs[0] - s1) <= d:
            ret += 1
        elif abs(s0 - curs[0] - s1) <= d:
            ret += 1
        return ret
    else:
        ret += dfs(curs[1:], s0 + curs[0], s1, d)
        ret += dfs(curs[1:], s0, s1 + curs[0], d)
        return ret
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            res, cos = helper_align(skeleton_idx + 1, code_idx + 1)
            res = skel_char + res
            possibilities.append((res, cos))
        # Insert
        res, cos = helper_align(skeleton_idx, code_idx + 1)
        res = "+[" + code_char + "]" + res
        possibilities.append((res, cos + 1))
        # Delete
        res, cos = helper_align(skeleton_idx + 1, code_idx)
        res = "-[" + skel_char + "]" + res
        possibilities.append((res, cos + 1))
