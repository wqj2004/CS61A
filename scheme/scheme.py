...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        opt = scheme_eval(expr.first, env)
        f = lambda x: scheme_eval(x, env)
        args = expr.rest.map(f)
        return scheme_apply(opt, args, env)
        # END PROBLEM 4
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # 非内置过程，可能是自定义函数，就需要创建一个新的frame,然后再计算
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        exp = expressions.rest.first
        v = scheme_eval(exp, env)
        env.define(sym, v)
        return sym
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    # print("DEBUG:", "let_frame", bindings)
    while bindings != nil:
        bind = bindings.first
