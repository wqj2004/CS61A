...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    rollnum, flag, ret = 0, 0, 0
    while rollnum < num_rolls:
        x = dice()
        if x == 1:
            flag = 1
        ret += x
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    cur, k, ret = 0, 1, 0
    while k <= 10:
        kfunc = make_averaged(roll_dice, trials_count)
        kval = kfunc(k, dice)
        if kval > cur:
            cur = kval
            ret = k
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    temscore = score + free_bacon(opponent_score)
    if extra_turn(temscore, opponent_score):
        return 0
    else:
